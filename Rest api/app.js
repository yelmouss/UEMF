...
const configServer = (request, response) => {
  ...
  if (request.method == "DELETE" && request.url.startsWith("/users/")) {
    ...
    fs.promises
      .readFile("./database.json") // Lecture du fichier database.json
      ...
      .then((data) => {
        // data est maintenant l'objet users du fichier database.json
        ...
        if (resultat) {
          ...
          fs.promises
            .writeFile("./database.json", JSON.stringify(data, null, 3)) // Écriture des nouvelles données dans le fichier
            .then(() => {
  ...
  // GET /users => http://localhost:3000/users?limit=5
    ...
    if (request.url.split("?").length != 1) {
      // Vérification de la présence de paramètres de requête
    ...
    fs.promises
      ...
      .then((data) => {
        if (limit == -1 || limit > data.users.length)
          // Vérification de la validité de la limite
        ...
        response.setHeader("Content-Type", "application/json"); // Définition du type de contenu de la réponse
        response.write(
          JSON.stringify(data.users.filter((val, index) => index < limit))
        ); // Écriture des utilisateurs dans la réponse, en respectant la limite
      })
      .catch((e) => {
    ...
    let body = ""; // Initialisation du corps de la requête
    request.on("data", (chunk) => {
      // Événement de réception de données
      body += chunk.toString(); // Ajout des données reçues au corps de la requête
    });
    request.on("end", () => {
      // Événement de fin de réception de données
      let user = JSON.parse(body); // Conversion du corps de la requête en objet JavaScript
      fs.promises
        .readFile("./database.json") // Lecture du fichier database.json
        .then((data) => data.toString()) // Conversion des données en chaîne de caractères
        .then((data) => JSON.parse(data)) // Conversion de la chaîne JSON en objet JavaScript
        .then((data) => {
          user.id = data.users.length + 1; // Attribution d'un ID à l'utilisateur
          data.users.push(user); // Ajout de l'utilisateur à la liste des utilisateurs
          fs.promises
            .writeFile("./database.json", JSON.stringify(data, null, 3)) // Écriture des nouvelles données dans le fichier
            .then(() => {
              response.setHeader("Content-Type", "application/json"); // Définition du type de contenu de la réponse
              response.statusCode = 201; // Code de statut HTTP 201 (Créé)
              response.write(JSON.stringify(user)); // Écriture de l'utilisateur ajouté dans la réponse
              response.end(); // Fin de la réponse
            })
            .catch((e) => {
              console.log(e); // Affichage de l'erreur dans la console
    ...
    let body = "";
    request.on("data", (chunk) => {
      body += chunk.toString(); // Accumulation des données du corps de la requête
    });
    request.on("end", () => {
      let updatedUser = JSON.parse(body); // Conversion des données en objet JavaScript
      fs.promises
        .readFile("./database.json") // Lecture du fichier database.json
        .then((data) => data.toString()) // Conversion des données en chaîne de caractères
        .then((data) => JSON.parse(data)) // Conversion de la chaîne JSON en objet JavaScript
        .then((data) => {
          let index = data.users.findIndex((value) => value.id == id); // Recherche de l'utilisateur par ID
          if (index !== -1) {
            data.users[index] = updatedUser; // Mise à jour de l'utilisateur
            fs.promises
              .writeFile("./database.json", JSON.stringify(data, null, 3)) // Écriture des nouvelles données dans le fichier
              .then(() => {
                response.setHeader("Content-Type", "application/json"); // Définition du type de contenu de la réponse
                response.statusCode = 200; // Code de statut HTTP 200 (OK)
                response.write(JSON.stringify(updatedUser)); // Écriture de l'utilisateur mis à jour dans la réponse
                response.end(); // Fin de la réponse
              })
              .catch((e) => {
                console.log(e); // Affichage de l'erreur dans la console
